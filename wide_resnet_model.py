...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=100, zero_init_residual=False,
        ...
        self.fusion3_4 = AFF(channels=512 * block.expansion)

        self.downsample3_4 = nn.Sequential(
            nn.Conv2d(256 * block.expansion, 256 * block.expansion, 3, 2, 1, groups=256 * block.expansion),
            nn.Conv2d(256 * block.expansion, 512 * block.expansion, 1, 1, 0),
        ...
        self.attention1 = nn.Sequential(
            nn.Conv2d(64 * block.expansion, 64 * block.expansion // 4, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(64 * block.expansion // 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(64 * block.expansion // 4, 64 * block.expansion, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(64 * block.expansion),
        )
        self.attention2 = nn.Sequential(
            nn.Conv2d(128 * block.expansion, 128 * block.expansion // 4, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(128 * block.expansion // 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(128 * block.expansion // 4, 128 * block.expansion, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(128 * block.expansion),
        )
        self.attention3 = nn.Sequential(
            nn.Conv2d(256 * block.expansion, 256 * block.expansion // 4, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(256 * block.expansion // 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(256 * block.expansion // 4, 256 * block.expansion, kernel_size=1, stride=1, padding=0),
...
def wide_resnet38_1(pretrained=False, progress=True, **kwargs):
    r"""Wide ResNet-50-2 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    kwargs['width_per_group'] = 64 * 1
    return _resnet('wide_resnet38_1', Bottleneck, [2, 3, 5, 2],
                   pretrained, progress, **kwargs)
