...
learning_rate = 0.1
save_path = './r18.pth'
logger = log_creater('./logger/r18.log')
...
def self_kd_main():
    from wide_resnet_model import resnet18

    self_kd = resnet18(num_classes=100)
    self_kd.to(device)

    batch_size = 128
    setup_seed(100)
    optimizer = optim.SGD(self_kd.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
    train_loader, test_loader = get_cifar100_dataloader(batch_size)
...
if __name__ == '__main__':
    ...
    logger.info('the best acc and avg acc of branch4 are {} and {}'.format(best_acc_branch4,
                                                                           avg_acc_branch4))
    logger.info('the best acc of ensemble2, ensemble3 and ensemble4 are {}, {} and {}'.format(best_acc_ensemble4,
                                                                                              best_acc_ensemble3,
                                                                                              best_acc_ensemble2))
